...
async def main():
    ...
    try:
        ...
        ath, drawdown = await data_center.get_token_call_performance(token_address, chain='sol', called_time=datetime.datetime.now() - datetime.timedelta(hours=3))
        logger.info(f"ATH: {ath}, Drawdown: {drawdown}")
        ...
        # Example 5: Search logs (Note: This requires OpenSearch to be set up)
        # logs = await data_center.search_logs("your_index", 12345678, 12345700, ["0x..."])
